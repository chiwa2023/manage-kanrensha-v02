...
/**
 * DumpKanrenshaPersonHistory31ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory31ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_31.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory31ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory31Entity entity00 = new KanrenshaPersonHistory31Entity();
        ...
        List<KanrenshaPersonHistory31Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory31Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory31ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory31ItemWriter.write(items);
