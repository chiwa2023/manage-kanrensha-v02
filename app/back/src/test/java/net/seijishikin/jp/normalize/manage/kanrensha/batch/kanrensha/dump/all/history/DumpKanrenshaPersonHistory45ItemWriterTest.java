...
/**
 * DumpKanrenshaPersonHistory45ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory45ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_45.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory45ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory45Entity entity00 = new KanrenshaPersonHistory45Entity();
        ...
        List<KanrenshaPersonHistory45Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory45Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory45ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory45ItemWriter.write(items);
