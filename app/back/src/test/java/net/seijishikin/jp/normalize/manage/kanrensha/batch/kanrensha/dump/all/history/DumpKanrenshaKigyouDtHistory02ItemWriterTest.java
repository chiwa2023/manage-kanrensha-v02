...
/**
 * DumpKanrenshaKigyouDtHistory02ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory02ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_02.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory02ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory02Entity entity00 = new KanrenshaKigyouDtHistory02Entity();
        ...
        List<KanrenshaKigyouDtHistory02Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory02Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory02ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory02ItemWriter.write(items);
