...
/**
 * DumpKanrenshaKigyouDtHistory19ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory19ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_19.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory19ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory19Entity entity00 = new KanrenshaKigyouDtHistory19Entity();
        ...
        List<KanrenshaKigyouDtHistory19Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory19Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory19ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory19ItemWriter.write(items);
