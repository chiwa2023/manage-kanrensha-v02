...
/**
 * DumpKanrenshaPersonHistory36ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory36ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_36.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory36ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory36Entity entity00 = new KanrenshaPersonHistory36Entity();
        ...
        List<KanrenshaPersonHistory36Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory36Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory36ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory36ItemWriter.write(items);
