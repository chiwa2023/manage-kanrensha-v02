...
/**
 * DumpKanrenshaKigyouDtHistory45ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory45ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_45.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory45ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory45Entity entity00 = new KanrenshaKigyouDtHistory45Entity();
        ...
        List<KanrenshaKigyouDtHistory45Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory45Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory45ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory45ItemWriter.write(items);
