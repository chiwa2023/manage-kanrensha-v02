...
/**
 * DumpKanrenshaKigyouDtHistory16ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory16ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_16.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory16ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory16Entity entity00 = new KanrenshaKigyouDtHistory16Entity();
        ...
        List<KanrenshaKigyouDtHistory16Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory16Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory16ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory16ItemWriter.write(items);
