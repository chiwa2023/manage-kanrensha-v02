...
/**
 * DumpKanrenshaKigyouDtHistory41ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory41ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_41.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory41ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory41Entity entity00 = new KanrenshaKigyouDtHistory41Entity();
        ...
        List<KanrenshaKigyouDtHistory41Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory41Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory41ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory41ItemWriter.write(items);
