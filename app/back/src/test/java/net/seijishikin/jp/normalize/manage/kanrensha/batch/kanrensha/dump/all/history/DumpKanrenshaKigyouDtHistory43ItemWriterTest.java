...
/**
 * DumpKanrenshaKigyouDtHistory43ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory43ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_43.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory43ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory43Entity entity00 = new KanrenshaKigyouDtHistory43Entity();
        ...
        List<KanrenshaKigyouDtHistory43Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory43Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory43ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory43ItemWriter.write(items);
