...
/**
 * DumpKanrenshaKigyouDtHistory15ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory15ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_15.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory15ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory15Entity entity00 = new KanrenshaKigyouDtHistory15Entity();
        ...
        List<KanrenshaKigyouDtHistory15Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory15Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory15ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory15ItemWriter.write(items);
