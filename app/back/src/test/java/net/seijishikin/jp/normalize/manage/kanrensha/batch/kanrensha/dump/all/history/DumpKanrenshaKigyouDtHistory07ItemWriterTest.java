...
/**
 * DumpKanrenshaKigyouDtHistory07ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory07ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_07.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory07ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory07Entity entity00 = new KanrenshaKigyouDtHistory07Entity();
        ...
        List<KanrenshaKigyouDtHistory07Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory07Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory07ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory07ItemWriter.write(items);
