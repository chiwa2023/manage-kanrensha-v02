...
/**
 * DumpKanrenshaKigyouDtHistory42ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory42ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_42.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory42ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory42Entity entity00 = new KanrenshaKigyouDtHistory42Entity();
        ...
        List<KanrenshaKigyouDtHistory42Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory42Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory42ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory42ItemWriter.write(items);
