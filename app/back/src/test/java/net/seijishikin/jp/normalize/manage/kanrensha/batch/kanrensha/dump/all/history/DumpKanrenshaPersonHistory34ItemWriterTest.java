...
/**
 * DumpKanrenshaPersonHistory34ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory34ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_34.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory34ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory34Entity entity00 = new KanrenshaPersonHistory34Entity();
        ...
        List<KanrenshaPersonHistory34Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory34Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory34ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory34ItemWriter.write(items);
