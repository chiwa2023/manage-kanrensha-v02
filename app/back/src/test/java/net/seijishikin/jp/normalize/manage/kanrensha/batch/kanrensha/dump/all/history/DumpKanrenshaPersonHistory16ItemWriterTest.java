...
/**
 * DumpKanrenshaPersonHistory16ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory16ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_16.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory16ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory16Entity entity00 = new KanrenshaPersonHistory16Entity();
        ...
        List<KanrenshaPersonHistory16Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory16Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory16ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory16ItemWriter.write(items);
