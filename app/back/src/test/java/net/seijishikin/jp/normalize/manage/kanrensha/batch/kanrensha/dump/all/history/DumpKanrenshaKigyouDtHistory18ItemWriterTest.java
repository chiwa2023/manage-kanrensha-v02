...
/**
 * DumpKanrenshaKigyouDtHistory18ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory18ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_18.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory18ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory18Entity entity00 = new KanrenshaKigyouDtHistory18Entity();
        ...
        List<KanrenshaKigyouDtHistory18Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory18Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory18ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory18ItemWriter.write(items);
