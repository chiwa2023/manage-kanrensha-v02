...
/**
 * DumpKanrenshaKigyouDtHistory27ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory27ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_27.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory27ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory27Entity entity00 = new KanrenshaKigyouDtHistory27Entity();
        ...
        List<KanrenshaKigyouDtHistory27Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory27Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory27ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory27ItemWriter.write(items);
