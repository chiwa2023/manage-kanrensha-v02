...
/**
 * DumpKanrenshaKigyouDtHistory28ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory28ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_28.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory28ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory28Entity entity00 = new KanrenshaKigyouDtHistory28Entity();
        ...
        List<KanrenshaKigyouDtHistory28Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory28Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory28ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory28ItemWriter.write(items);
