...
/**
 * DumpKanrenshaPersonHistory14ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory14ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_14.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory14ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory14Entity entity00 = new KanrenshaPersonHistory14Entity();
        ...
        List<KanrenshaPersonHistory14Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory14Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory14ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory14ItemWriter.write(items);
