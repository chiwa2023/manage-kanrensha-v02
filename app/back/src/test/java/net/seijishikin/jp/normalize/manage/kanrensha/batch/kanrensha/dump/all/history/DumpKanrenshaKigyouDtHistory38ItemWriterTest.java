...
/**
 * DumpKanrenshaKigyouDtHistory38ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory38ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_38.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory38ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory38Entity entity00 = new KanrenshaKigyouDtHistory38Entity();
        ...
        List<KanrenshaKigyouDtHistory38Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory38Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory38ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory38ItemWriter.write(items);
