...
/**
 * DumpKanrenshaPersonHistory26ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory26ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_26.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory26ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory26Entity entity00 = new KanrenshaPersonHistory26Entity();
        ...
        List<KanrenshaPersonHistory26Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory26Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory26ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory26ItemWriter.write(items);
