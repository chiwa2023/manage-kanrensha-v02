...
/**
 * DumpKanrenshaKigyouDtHistory34ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory34ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_34.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory34ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory34Entity entity00 = new KanrenshaKigyouDtHistory34Entity();
        ...
        List<KanrenshaKigyouDtHistory34Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory34Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory34ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory34ItemWriter.write(items);
