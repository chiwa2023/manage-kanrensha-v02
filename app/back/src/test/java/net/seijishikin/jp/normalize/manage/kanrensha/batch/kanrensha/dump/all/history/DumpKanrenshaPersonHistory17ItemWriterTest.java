...
/**
 * DumpKanrenshaPersonHistory17ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory17ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_17.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory17ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory17Entity entity00 = new KanrenshaPersonHistory17Entity();
        ...
        List<KanrenshaPersonHistory17Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory17Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory17ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory17ItemWriter.write(items);
