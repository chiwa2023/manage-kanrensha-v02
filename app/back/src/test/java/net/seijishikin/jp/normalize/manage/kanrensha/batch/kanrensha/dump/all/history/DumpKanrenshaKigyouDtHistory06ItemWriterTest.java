...
/**
 * DumpKanrenshaKigyouDtHistory06ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory06ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_06.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory06ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory06Entity entity00 = new KanrenshaKigyouDtHistory06Entity();
        ...
        List<KanrenshaKigyouDtHistory06Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory06Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory06ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory06ItemWriter.write(items);
