...
/**
 * DumpKanrenshaPersonHistory11ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory11ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_11.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory11ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory11Entity entity00 = new KanrenshaPersonHistory11Entity();
        ...
        List<KanrenshaPersonHistory11Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory11Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory11ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory11ItemWriter.write(items);
