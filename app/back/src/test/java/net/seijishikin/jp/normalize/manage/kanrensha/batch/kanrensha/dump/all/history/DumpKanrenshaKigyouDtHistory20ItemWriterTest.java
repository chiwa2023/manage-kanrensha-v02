...
/**
 * DumpKanrenshaKigyouDtHistory20ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory20ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_20.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory20ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory20Entity entity00 = new KanrenshaKigyouDtHistory20Entity();
        ...
        List<KanrenshaKigyouDtHistory20Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory20Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory20ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory20ItemWriter.write(items);
