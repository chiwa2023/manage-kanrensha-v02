...
/**
 * DumpKanrenshaPersonHistory39ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory39ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_39.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory39ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory39Entity entity00 = new KanrenshaPersonHistory39Entity();
        ...
        List<KanrenshaPersonHistory39Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory39Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory39ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory39ItemWriter.write(items);
