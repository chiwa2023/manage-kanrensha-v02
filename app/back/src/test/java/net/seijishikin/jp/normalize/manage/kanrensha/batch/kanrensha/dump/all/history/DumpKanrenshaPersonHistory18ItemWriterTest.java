...
/**
 * DumpKanrenshaPersonHistory18ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory18ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_18.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory18ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory18Entity entity00 = new KanrenshaPersonHistory18Entity();
        ...
        List<KanrenshaPersonHistory18Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory18Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory18ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory18ItemWriter.write(items);
