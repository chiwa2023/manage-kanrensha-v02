...
/**
 * DumpKanrenshaKigyouDtHistory14ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory14ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_14.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory14ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory14Entity entity00 = new KanrenshaKigyouDtHistory14Entity();
        ...
        List<KanrenshaKigyouDtHistory14Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory14Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory14ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory14ItemWriter.write(items);
