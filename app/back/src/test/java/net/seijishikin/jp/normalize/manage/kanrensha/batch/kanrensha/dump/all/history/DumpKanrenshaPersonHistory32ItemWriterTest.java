...
/**
 * DumpKanrenshaPersonHistory32ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory32ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_32.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory32ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory32Entity entity00 = new KanrenshaPersonHistory32Entity();
        ...
        List<KanrenshaPersonHistory32Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory32Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory32ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory32ItemWriter.write(items);
