...
/**
 * DumpKanrenshaPersonHistory27ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory27ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_27.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory27ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory27Entity entity00 = new KanrenshaPersonHistory27Entity();
        ...
        List<KanrenshaPersonHistory27Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory27Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory27ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory27ItemWriter.write(items);
