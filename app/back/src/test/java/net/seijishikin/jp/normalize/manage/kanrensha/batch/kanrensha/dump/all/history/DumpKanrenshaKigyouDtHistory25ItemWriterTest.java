...
/**
 * DumpKanrenshaKigyouDtHistory25ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory25ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_25.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory25ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory25Entity entity00 = new KanrenshaKigyouDtHistory25Entity();
        ...
        List<KanrenshaKigyouDtHistory25Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory25Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory25ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory25ItemWriter.write(items);
