...
/**
 * DumpKanrenshaPersonHistory13ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory13ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_13.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory13ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory13Entity entity00 = new KanrenshaPersonHistory13Entity();
        ...
        List<KanrenshaPersonHistory13Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory13Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory13ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory13ItemWriter.write(items);
