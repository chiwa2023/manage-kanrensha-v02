...
/**
 * DumpKanrenshaPersonHistory37ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory37ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_37.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory37ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory37Entity entity00 = new KanrenshaPersonHistory37Entity();
        ...
        List<KanrenshaPersonHistory37Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory37Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory37ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory37ItemWriter.write(items);
