...
/**
 * DumpKanrenshaKigyouDtHistory30ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory30ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_30.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory30ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory30Entity entity00 = new KanrenshaKigyouDtHistory30Entity();
        ...
        List<KanrenshaKigyouDtHistory30Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory30Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory30ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory30ItemWriter.write(items);
