...
/**
 * DumpKanrenshaPersonHistory33ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory33ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_33.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory33ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory33Entity entity00 = new KanrenshaPersonHistory33Entity();
        ...
        List<KanrenshaPersonHistory33Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory33Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory33ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory33ItemWriter.write(items);
