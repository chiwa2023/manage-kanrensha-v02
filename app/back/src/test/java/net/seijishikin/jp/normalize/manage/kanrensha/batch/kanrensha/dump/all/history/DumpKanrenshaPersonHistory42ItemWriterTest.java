...
/**
 * DumpKanrenshaPersonHistory42ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory42ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_42.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory42ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory42Entity entity00 = new KanrenshaPersonHistory42Entity();
        ...
        List<KanrenshaPersonHistory42Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory42Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory42ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory42ItemWriter.write(items);
