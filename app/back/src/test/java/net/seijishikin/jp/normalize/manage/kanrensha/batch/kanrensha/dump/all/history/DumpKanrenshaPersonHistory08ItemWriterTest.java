...
/**
 * DumpKanrenshaPersonHistory08ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory08ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_08.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory08ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory08Entity entity00 = new KanrenshaPersonHistory08Entity();
        ...
        List<KanrenshaPersonHistory08Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory08Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory08ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory08ItemWriter.write(items);
