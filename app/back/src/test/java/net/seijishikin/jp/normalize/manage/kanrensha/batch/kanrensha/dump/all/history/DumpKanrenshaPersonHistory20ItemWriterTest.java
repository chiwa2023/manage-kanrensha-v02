...
/**
 * DumpKanrenshaPersonHistory20ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory20ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_20.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory20ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory20Entity entity00 = new KanrenshaPersonHistory20Entity();
        ...
        List<KanrenshaPersonHistory20Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory20Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory20ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory20ItemWriter.write(items);
