...
/**
 * DumpKanrenshaKigyouDtHistory44ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory44ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_44.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory44ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory44Entity entity00 = new KanrenshaKigyouDtHistory44Entity();
        ...
        List<KanrenshaKigyouDtHistory44Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory44Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory44ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory44ItemWriter.write(items);
