...
/**
 * DumpKanrenshaKigyouDtHistory35ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory35ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_35.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory35ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory35Entity entity00 = new KanrenshaKigyouDtHistory35Entity();
        ...
        List<KanrenshaKigyouDtHistory35Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory35Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory35ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory35ItemWriter.write(items);
