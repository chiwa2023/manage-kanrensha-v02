...
/**
 * DumpKanrenshaPersonHistory38ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory38ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_38.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory38ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory38Entity entity00 = new KanrenshaPersonHistory38Entity();
        ...
        List<KanrenshaPersonHistory38Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory38Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory38ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory38ItemWriter.write(items);
