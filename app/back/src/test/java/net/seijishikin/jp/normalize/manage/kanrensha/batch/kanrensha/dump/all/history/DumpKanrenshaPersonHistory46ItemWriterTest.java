...
/**
 * DumpKanrenshaPersonHistory46ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory46ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_46.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory46ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory46Entity entity00 = new KanrenshaPersonHistory46Entity();
        ...
        List<KanrenshaPersonHistory46Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory46Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory46ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory46ItemWriter.write(items);
