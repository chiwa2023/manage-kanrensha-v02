...
/**
 * DumpKanrenshaPersonHistory35ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory35ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_35.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory35ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory35Entity entity00 = new KanrenshaPersonHistory35Entity();
        ...
        List<KanrenshaPersonHistory35Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory35Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory35ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory35ItemWriter.write(items);
