...
/**
 * DumpKanrenshaKigyouDtHistory08ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory08ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_08.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory08ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory08Entity entity00 = new KanrenshaKigyouDtHistory08Entity();
        ...
        List<KanrenshaKigyouDtHistory08Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory08Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory08ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory08ItemWriter.write(items);
