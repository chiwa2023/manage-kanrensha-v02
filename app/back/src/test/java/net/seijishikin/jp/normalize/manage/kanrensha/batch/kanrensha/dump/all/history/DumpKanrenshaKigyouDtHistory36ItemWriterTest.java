...
/**
 * DumpKanrenshaKigyouDtHistory36ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory36ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_36.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory36ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory36Entity entity00 = new KanrenshaKigyouDtHistory36Entity();
        ...
        List<KanrenshaKigyouDtHistory36Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory36Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory36ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory36ItemWriter.write(items);
