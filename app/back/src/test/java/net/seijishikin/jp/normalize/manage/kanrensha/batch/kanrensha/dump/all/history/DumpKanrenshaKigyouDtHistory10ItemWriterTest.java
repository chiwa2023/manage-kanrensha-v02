...
/**
 * DumpKanrenshaKigyouDtHistory10ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory10ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_10.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory10ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory10Entity entity00 = new KanrenshaKigyouDtHistory10Entity();
        ...
        List<KanrenshaKigyouDtHistory10Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory10Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory10ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory10ItemWriter.write(items);
