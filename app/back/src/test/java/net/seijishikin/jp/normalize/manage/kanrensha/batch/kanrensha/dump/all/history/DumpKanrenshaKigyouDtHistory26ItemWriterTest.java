...
/**
 * DumpKanrenshaKigyouDtHistory26ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory26ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_26.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory26ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory26Entity entity00 = new KanrenshaKigyouDtHistory26Entity();
        ...
        List<KanrenshaKigyouDtHistory26Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory26Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory26ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory26ItemWriter.write(items);
