...
/**
 * DumpKanrenshaKigyouDtHistory22ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory22ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_22.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory22ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory22Entity entity00 = new KanrenshaKigyouDtHistory22Entity();
        ...
        List<KanrenshaKigyouDtHistory22Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory22Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory22ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory22ItemWriter.write(items);
