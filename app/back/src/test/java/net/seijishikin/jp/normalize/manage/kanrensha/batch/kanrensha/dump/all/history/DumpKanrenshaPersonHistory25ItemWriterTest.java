...
/**
 * DumpKanrenshaPersonHistory25ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory25ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_25.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory25ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory25Entity entity00 = new KanrenshaPersonHistory25Entity();
        ...
        List<KanrenshaPersonHistory25Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory25Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory25ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory25ItemWriter.write(items);
