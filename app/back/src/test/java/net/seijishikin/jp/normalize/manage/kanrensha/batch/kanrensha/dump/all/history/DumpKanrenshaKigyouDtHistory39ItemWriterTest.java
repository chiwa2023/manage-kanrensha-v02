...
/**
 * DumpKanrenshaKigyouDtHistory39ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory39ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_39.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory39ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory39Entity entity00 = new KanrenshaKigyouDtHistory39Entity();
        ...
        List<KanrenshaKigyouDtHistory39Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory39Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory39ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory39ItemWriter.write(items);
