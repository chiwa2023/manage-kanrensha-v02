...
/**
 * DumpKanrenshaPersonHistory22ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory22ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_22.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory22ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory22Entity entity00 = new KanrenshaPersonHistory22Entity();
        ...
        List<KanrenshaPersonHistory22Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory22Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory22ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory22ItemWriter.write(items);
