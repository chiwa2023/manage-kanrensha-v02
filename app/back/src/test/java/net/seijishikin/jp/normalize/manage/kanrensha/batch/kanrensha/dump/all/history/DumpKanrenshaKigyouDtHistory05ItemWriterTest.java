...
/**
 * DumpKanrenshaKigyouDtHistory05ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory05ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_05.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory05ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory05Entity entity00 = new KanrenshaKigyouDtHistory05Entity();
        ...
        List<KanrenshaKigyouDtHistory05Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory05Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory05ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory05ItemWriter.write(items);
