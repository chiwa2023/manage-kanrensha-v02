...
/**
 * DumpKanrenshaKigyouDtHistory23ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory23ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_23.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory23ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory23Entity entity00 = new KanrenshaKigyouDtHistory23Entity();
        ...
        List<KanrenshaKigyouDtHistory23Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory23Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory23ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory23ItemWriter.write(items);
