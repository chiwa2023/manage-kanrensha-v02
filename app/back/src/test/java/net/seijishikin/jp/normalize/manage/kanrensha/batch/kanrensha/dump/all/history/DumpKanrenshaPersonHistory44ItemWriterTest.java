...
/**
 * DumpKanrenshaPersonHistory44ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory44ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_44.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory44ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory44Entity entity00 = new KanrenshaPersonHistory44Entity();
        ...
        List<KanrenshaPersonHistory44Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory44Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory44ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory44ItemWriter.write(items);
