...
/**
 * DumpKanrenshaKigyouDtHistory31ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory31ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_31.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory31ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory31Entity entity00 = new KanrenshaKigyouDtHistory31Entity();
        ...
        List<KanrenshaKigyouDtHistory31Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory31Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory31ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory31ItemWriter.write(items);
