...
/**
 * DumpKanrenshaPersonHistory47ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory47ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_47.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory47ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory47Entity entity00 = new KanrenshaPersonHistory47Entity();
        ...
        List<KanrenshaPersonHistory47Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory47Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory47ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory47ItemWriter.write(items);
