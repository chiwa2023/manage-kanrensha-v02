...
/**
 * DumpKanrenshaKigyouDtHistory33ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory33ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_33.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory33ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory33Entity entity00 = new KanrenshaKigyouDtHistory33Entity();
        ...
        List<KanrenshaKigyouDtHistory33Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory33Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory33ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory33ItemWriter.write(items);
