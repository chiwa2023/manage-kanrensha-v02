...
/**
 * DumpKanrenshaPersonHistory12ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory12ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_12.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory12ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory12Entity entity00 = new KanrenshaPersonHistory12Entity();
        ...
        List<KanrenshaPersonHistory12Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory12Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory12ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory12ItemWriter.write(items);
