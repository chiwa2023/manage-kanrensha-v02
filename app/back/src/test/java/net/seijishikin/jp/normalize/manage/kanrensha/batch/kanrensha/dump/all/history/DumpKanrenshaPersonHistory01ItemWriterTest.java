...
/**
 * DumpKanrenshaPersonHistory01ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory01ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_01.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory01ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory01Entity entity00 = new KanrenshaPersonHistory01Entity();
        ...
        List<KanrenshaPersonHistory01Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory01Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory01ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory01ItemWriter.write(items);
