...
/**
 * DumpKanrenshaPersonHistory24ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory24ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_24.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory24ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory24Entity entity00 = new KanrenshaPersonHistory24Entity();
        ...
        List<KanrenshaPersonHistory24Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory24Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory24ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory24ItemWriter.write(items);
