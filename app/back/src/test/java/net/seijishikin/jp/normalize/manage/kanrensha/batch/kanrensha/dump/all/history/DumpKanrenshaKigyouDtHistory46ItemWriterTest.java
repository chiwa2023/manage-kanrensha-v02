...
/**
 * DumpKanrenshaKigyouDtHistory46ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory46ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_46.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory46ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory46Entity entity00 = new KanrenshaKigyouDtHistory46Entity();
        ...
        List<KanrenshaKigyouDtHistory46Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory46Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory46ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory46ItemWriter.write(items);
