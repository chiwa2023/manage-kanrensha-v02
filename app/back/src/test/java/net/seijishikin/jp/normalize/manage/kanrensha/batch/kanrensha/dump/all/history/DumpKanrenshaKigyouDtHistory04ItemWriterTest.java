...
/**
 * DumpKanrenshaKigyouDtHistory04ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory04ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_04.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory04ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory04Entity entity00 = new KanrenshaKigyouDtHistory04Entity();
        ...
        List<KanrenshaKigyouDtHistory04Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory04Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory04ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory04ItemWriter.write(items);
