...
/**
 * DumpKanrenshaPersonHistory02ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory02ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_02.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory02ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory02Entity entity00 = new KanrenshaPersonHistory02Entity();
        ...
        List<KanrenshaPersonHistory02Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory02Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory02ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory02ItemWriter.write(items);
