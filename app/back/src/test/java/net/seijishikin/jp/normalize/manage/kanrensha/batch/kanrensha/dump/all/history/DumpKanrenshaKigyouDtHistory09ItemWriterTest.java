...
/**
 * DumpKanrenshaKigyouDtHistory09ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory09ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_09.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory09ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory09Entity entity00 = new KanrenshaKigyouDtHistory09Entity();
        ...
        List<KanrenshaKigyouDtHistory09Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory09Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory09ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory09ItemWriter.write(items);
