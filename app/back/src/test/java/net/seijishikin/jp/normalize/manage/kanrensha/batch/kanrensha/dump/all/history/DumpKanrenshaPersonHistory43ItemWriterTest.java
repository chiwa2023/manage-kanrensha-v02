...
/**
 * DumpKanrenshaPersonHistory43ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory43ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_43.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory43ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory43Entity entity00 = new KanrenshaPersonHistory43Entity();
        ...
        List<KanrenshaPersonHistory43Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory43Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory43ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory43ItemWriter.write(items);
