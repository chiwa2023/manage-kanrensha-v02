...
/**
 * DumpKanrenshaPersonHistory99ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory99ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_99.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory99ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory99Entity entity00 = new KanrenshaPersonHistory99Entity();
        ...
        List<KanrenshaPersonHistory99Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory99Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory99ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory99ItemWriter.write(items);
