...
/**
 * DumpKanrenshaKigyouDtHistory37ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory37ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_37.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory37ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory37Entity entity00 = new KanrenshaKigyouDtHistory37Entity();
        ...
        List<KanrenshaKigyouDtHistory37Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory37Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory37ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory37ItemWriter.write(items);
