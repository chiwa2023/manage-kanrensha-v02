...
/**
 * DumpKanrenshaKigyouDtHistory99ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory99ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_99.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory99ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory99Entity entity00 = new KanrenshaKigyouDtHistory99Entity();
        ...
        List<KanrenshaKigyouDtHistory99Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory99Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory99ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory99ItemWriter.write(items);
