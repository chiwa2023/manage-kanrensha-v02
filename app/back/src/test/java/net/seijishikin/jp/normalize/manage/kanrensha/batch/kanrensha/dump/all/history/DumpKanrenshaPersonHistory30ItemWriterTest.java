...
/**
 * DumpKanrenshaPersonHistory30ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory30ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_30.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory30ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory30Entity entity00 = new KanrenshaPersonHistory30Entity();
        ...
        List<KanrenshaPersonHistory30Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory30Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory30ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory30ItemWriter.write(items);
