...
/**
 * DumpKanrenshaPersonHistory15ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory15ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_15.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory15ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory15Entity entity00 = new KanrenshaPersonHistory15Entity();
        ...
        List<KanrenshaPersonHistory15Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory15Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory15ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory15ItemWriter.write(items);
