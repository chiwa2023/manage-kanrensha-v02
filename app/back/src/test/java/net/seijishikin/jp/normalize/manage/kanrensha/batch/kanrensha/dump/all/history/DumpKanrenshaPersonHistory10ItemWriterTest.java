...
/**
 * DumpKanrenshaPersonHistory10ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory10ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_10.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory10ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory10Entity entity00 = new KanrenshaPersonHistory10Entity();
        ...
        List<KanrenshaPersonHistory10Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory10Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory10ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory10ItemWriter.write(items);
