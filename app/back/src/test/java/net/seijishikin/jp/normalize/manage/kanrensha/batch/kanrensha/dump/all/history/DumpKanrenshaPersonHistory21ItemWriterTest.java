...
/**
 * DumpKanrenshaPersonHistory21ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory21ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_21.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory21ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory21Entity entity00 = new KanrenshaPersonHistory21Entity();
        ...
        List<KanrenshaPersonHistory21Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory21Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory21ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory21ItemWriter.write(items);
