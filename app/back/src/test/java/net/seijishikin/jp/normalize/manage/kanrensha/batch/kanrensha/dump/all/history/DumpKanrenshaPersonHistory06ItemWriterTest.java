...
/**
 * DumpKanrenshaPersonHistory06ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory06ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_06.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory06ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory06Entity entity00 = new KanrenshaPersonHistory06Entity();
        ...
        List<KanrenshaPersonHistory06Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory06Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory06ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory06ItemWriter.write(items);
