...
/**
 * DumpKanrenshaPersonHistory23ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory23ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_23.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory23ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory23Entity entity00 = new KanrenshaPersonHistory23Entity();
        ...
        List<KanrenshaPersonHistory23Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory23Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory23ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory23ItemWriter.write(items);
