...
/**
 * DumpKanrenshaKigyouDtHistory21ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory21ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_21.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory21ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory21Entity entity00 = new KanrenshaKigyouDtHistory21Entity();
        ...
        List<KanrenshaKigyouDtHistory21Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory21Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory21ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory21ItemWriter.write(items);
