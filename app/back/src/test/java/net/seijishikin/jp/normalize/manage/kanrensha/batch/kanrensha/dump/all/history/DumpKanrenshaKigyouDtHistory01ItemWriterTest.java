...
/**
 * DumpKanrenshaKigyouDtHistory01ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory01ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_01.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory01ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory01Entity entity00 = new KanrenshaKigyouDtHistory01Entity();
        ...
        List<KanrenshaKigyouDtHistory01Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory01Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory01ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory01ItemWriter.write(items);
