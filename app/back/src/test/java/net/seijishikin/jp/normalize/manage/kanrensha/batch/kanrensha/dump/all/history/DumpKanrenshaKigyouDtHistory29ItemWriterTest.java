...
/**
 * DumpKanrenshaKigyouDtHistory29ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory29ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_29.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory29ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory29Entity entity00 = new KanrenshaKigyouDtHistory29Entity();
        ...
        List<KanrenshaKigyouDtHistory29Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory29Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory29ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory29ItemWriter.write(items);
