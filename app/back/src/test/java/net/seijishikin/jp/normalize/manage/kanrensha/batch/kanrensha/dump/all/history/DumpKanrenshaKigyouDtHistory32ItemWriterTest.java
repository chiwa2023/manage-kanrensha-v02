...
/**
 * DumpKanrenshaKigyouDtHistory32ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory32ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_32.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory32ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory32Entity entity00 = new KanrenshaKigyouDtHistory32Entity();
        ...
        List<KanrenshaKigyouDtHistory32Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory32Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory32ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory32ItemWriter.write(items);
