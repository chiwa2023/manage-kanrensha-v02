...
/**
 * DumpKanrenshaKigyouDtHistory17ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory17ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_17.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory17ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory17Entity entity00 = new KanrenshaKigyouDtHistory17Entity();
        ...
        List<KanrenshaKigyouDtHistory17Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory17Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory17ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory17ItemWriter.write(items);
