...
/**
 * DumpKanrenshaPersonHistory07ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory07ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_07.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory07ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory07Entity entity00 = new KanrenshaPersonHistory07Entity();
        ...
        List<KanrenshaPersonHistory07Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory07Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory07ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory07ItemWriter.write(items);
