...
/**
 * DumpKanrenshaKigyouDtHistory24ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory24ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_24.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory24ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory24Entity entity00 = new KanrenshaKigyouDtHistory24Entity();
        ...
        List<KanrenshaKigyouDtHistory24Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory24Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory24ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory24ItemWriter.write(items);
