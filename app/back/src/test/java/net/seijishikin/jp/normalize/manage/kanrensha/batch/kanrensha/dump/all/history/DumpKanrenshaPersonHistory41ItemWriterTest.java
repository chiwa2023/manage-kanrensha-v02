...
/**
 * DumpKanrenshaPersonHistory41ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory41ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_41.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory41ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory41Entity entity00 = new KanrenshaPersonHistory41Entity();
        ...
        List<KanrenshaPersonHistory41Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory41Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory41ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory41ItemWriter.write(items);
