...
/**
 * DumpKanrenshaPersonHistory05ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory05ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_05.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory05ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory05Entity entity00 = new KanrenshaPersonHistory05Entity();
        ...
        List<KanrenshaPersonHistory05Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory05Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory05ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory05ItemWriter.write(items);
