...
/**
 * DumpKanrenshaKigyouDtHistory12ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaKigyouDtHistory12ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/kigyou_dt_hisotry_12.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaKigyouDtHistory12ItemWriter.beforeStep(stepExecution);

        KanrenshaKigyouDtHistory12Entity entity00 = new KanrenshaKigyouDtHistory12Entity();
        ...
        List<KanrenshaKigyouDtHistory12Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaKigyouDtHistory12Entity> items = new Chunk<>(list);
        dumpKanrenshaKigyouDtHistory12ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaKigyouDtHistory12ItemWriter.write(items);
