...
/**
 * DumpKanrenshaPersonHistory04ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpKanrenshaPersonHistory04ItemWriterTest {
    ...
    void test() throws Exception {
        ...
        String output = "/test/dump/person_hisotry_04.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpKanrenshaPersonHistory04ItemWriter.beforeStep(stepExecution);

        KanrenshaPersonHistory04Entity entity00 = new KanrenshaPersonHistory04Entity();
        ...
        List<KanrenshaPersonHistory04Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends KanrenshaPersonHistory04Entity> items = new Chunk<>(list);
        dumpKanrenshaPersonHistory04ItemWriter.open(stepExecution.getExecutionContext());
        dumpKanrenshaPersonHistory04ItemWriter.write(items);
