...
public interface KanrenshaPersonHistory02Repository extends JpaRepository<KanrenshaPersonHistory02Entity, Integer> {
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT * FROM kanrensha_person_history_02"
            + " WHERE search_text LIKE ?1 AND is_latest=1", nativeQuery = true) // TODO MATCH AGAINST
    List<KanrenshaPersonHistory02Entity> findFullText(String searchWords);
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT * FROM kanrensha_person_history_02 " + " WHERE all_name = ?1 AND all_address = ?2 "
