...
public interface KanrenshaSeijidantaiHistory04Repository
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT * FROM kanrensha_seijidantai_history_04"
            + " WHERE search_text LIKE ?1 AND is_latest=1", nativeQuery = true) // TODO MATCH AGAINST
    List<KanrenshaSeijidantaiHistory04Entity> findFullText(String searchWords);
     ...
     * @return 検索結果
     */
    @Query(value = "SELECT * FROM kanrensha_seijidantai_history_04 " + " WHERE all_name = ?1 AND all_address = ?2 "
     ...
     * @return 検索結果
     */
    Page<KanrenshaSeijidantaiHistory04Entity> findByInsertTimestampGreaterThanEqualAndInsertTimestampLessThanAndIsLatest(
