 ...
 */
@Component
public class CallKigyouDtMasterEntityLogic {
     ...
     * 処理を行う
     *
     * @param kigyouDtKanrenshaCode 関連者個人コード
     * @return 個人連絡先情報
     */
    public KanrenshaKigyouDtMasterEntity practice(final String kigyouDtKanrenshaCode) {
        // データベースよりpersonKanrenshaCodeが一致し、かつテーブルIdの降順で、リストを取得
        List<KanrenshaKigyouDtMasterEntity> entities = kanrenshaKigyouDtMasterRepository
                .findByKigyouDtKanrenshaCodeOrderByKanrenshaKigyouDtMasterIdDesc(kigyouDtKanrenshaCode);

        // 取得したリストが空の場合、新たにEntityのインスタンスを作成し、personKanrenshaCodeだけを設定して返却
        if (entities.isEmpty()) {
            KanrenshaKigyouDtMasterEntity newEntity = new KanrenshaKigyouDtMasterEntity();
            newEntity.setKigyouDtKanrenshaCode(kigyouDtKanrenshaCode);
            return newEntity;
        }

        // 取得したリストをisLatestがtrueでフィルタ
        List<KanrenshaKigyouDtMasterEntity> latestEntities = entities.stream()
                .filter(KanrenshaKigyouDtMasterEntity::getIsLatest).collect(Collectors.toList());
        ...
        if (nomalCnt < latestEntities.size()) {
            throw new DataRetrievalFailureException(
                    "Multiple latest records found for personKanrenshaCode: " + kigyouDtKanrenshaCode);
        }
