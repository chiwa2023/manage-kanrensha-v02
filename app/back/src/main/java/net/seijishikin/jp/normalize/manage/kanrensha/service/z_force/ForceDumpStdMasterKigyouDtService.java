...
/**
 * 企業／団体標準データをダンプする非同期Service
 */
@Service
@ConfigurationProperties(prefix = "net.seijishikin.jp.normalize.kanrensha")
public class ForceDumpStdMasterKigyouDtService {
    ...
    @Autowired
    private Job dumpMasterKigyouDtoration;
    ...
    public void practice(final Integer year, final InsertTaskPlanResultDto planDto, final LocalDate endDate,
        ...
        JobParameters jobParameters = new JobParametersBuilder(
                dumpMasterKigyouDtoration.getJobParametersIncrementer().getNext(new JobParameters())) // NOPMD
                .addLocalDateTime("executeTime", LocalDateTime.now())
                .addLocalDateTime("datetimeEnd", LocalDateTime.of(endDate.plusDays(1L), LocalTime.MIN))
                .addString("writeFilePath", Paths.get(pathSaved, folder, MasterStd.MASTER_STD_KIGYOU).toString())
        ...
        try {
            jobLauncher.run(dumpMasterKigyouDtoration, jobParameters);
