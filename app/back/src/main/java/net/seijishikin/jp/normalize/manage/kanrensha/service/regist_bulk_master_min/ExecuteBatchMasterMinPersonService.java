...
import net.seijishikin.jp.normalize.manage.kanrensha.batch.kanrensha.person.add_min.AddMinKanrenshaPersonMasterBatchConfiguration;
...
/**
 * Csv読み取り個人マスタ最小登録Service
 */
@Service
public class ExecuteBatchMasterMinPersonService {

    /** 起動をするJob */
    @Qualifier(AddMinKanrenshaPersonMasterBatchConfiguration.JOB_NAME)
    @Autowired
    private Job addMinKanrenshaPersonMaster;
    ...
    public void practice(final Integer year, final LeastUserDto userDto, final TaskPlanWithUseFileDto planFileDto) {
        ...
        JobParameters jobParameters = new JobParametersBuilder(
                addMinKanrenshaPersonMaster.getJobParametersIncrementer().getNext(new JobParameters())) // NOPMD
        ...
        try {
            jobLauncher.run(addMinKanrenshaPersonMaster, jobParameters);
