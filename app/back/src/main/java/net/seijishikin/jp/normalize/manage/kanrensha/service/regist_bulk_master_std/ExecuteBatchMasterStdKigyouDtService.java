...
import net.seijishikin.jp.normalize.manage.kanrensha.batch.kanrensha.kigyou_dt.add_std.AddStdKanrenshaKigyouDtMasterBatchConfiguration;
...
/**
 * Csv読み取り企業団体マスタ標準登録Service
 */
@Service
public class ExecuteBatchMasterStdKigyouDtService {

    /** 起動をするJob */
    @Qualifier(AddStdKanrenshaKigyouDtMasterBatchConfiguration.JOB_NAME)
    @Autowired
    private Job addStdKanrenshaKigyouDtorationMaster;
    ...
    public void practice(final Integer year, final LeastUserDto userDto, final TaskPlanWithUseFileDto planFileDto) {
        ...
        JobParameters jobParameters = new JobParametersBuilder(
                addStdKanrenshaKigyouDtorationMaster.getJobParametersIncrementer().getNext(new JobParameters())) // NOPMD
                .addLocalDateTime("executeTime", LocalDateTime.now())
                .addString("readFilePath", planFileDto.getReadFile().toString()) // NOPMD LawOfDemetersd
        ...
        try {
            jobLauncher.run(addStdKanrenshaKigyouDtorationMaster, jobParameters);
