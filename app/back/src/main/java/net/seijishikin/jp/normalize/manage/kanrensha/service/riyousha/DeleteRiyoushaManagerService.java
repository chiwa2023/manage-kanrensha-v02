 ...
 */
@Service
public class DeleteRiyoushaManagerService {
    ...
    public Integer practice(final Integer riyoushaCode, final LeastUserDto userDto) {
        ...
        List<RiyoushaCombineOrgEntity> listCombine = riyoushaCombineOrgRepository
                .findByPersonRiyoushaCodeAndRiyoushaRoleAndIsLatestTrue(riyoushaCode, UserRoleConstants.MANAGER);
        ...
        // 最新(1件しかないはず)マスタ全削除
        List<RiyoushaManagerMasterEntity> listMaster = riyoushaManagerMasterRepository
                .findByRiyoushaManagerMasterCodeAndIsLatestTrue(riyoushaCode);
        for (RiyoushaManagerMasterEntity entity : listMaster) {
            // 紐づく属性も削除
        ...
        if (sizeOnlyManager == listRole.size()) {
            ...
            for (UserRoleEntity entity : listRole) {
                if (UserRoleConstants.MANAGER.equals(entity.getRole())) {
                    setTableDataHistoryUtil.practiceDelete(userDto, entity);
        ...
        updateSize += riyoushaManagerMasterRepository.saveAll(listMaster).size();
        return updateSize;
    }
