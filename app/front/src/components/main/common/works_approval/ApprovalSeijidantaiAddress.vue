...
import type { KanrenshaSeijidantaiAddressEntityInterface } from '../../entity/kanrenshaSeijidantaiAddressEntity';
import mockGetSeijidantaiNoApprovalList from '../../../test/pages/works_approval/mock/mockGetPoliSeijidantaiApprovalList';
...
const listPersonAdsdress: Ref<KanrenshaSeijidantaiAddressEntityInterface[]> = ref([]);
function onSearch() {
...
function onPortalAddressInput(selectedId: number) {
    ...
    const tempEntity: KanrenshaSeijidantaiAddressEntityInterface | undefined
        = listPersonAdsdress.value.filter((e) => selectedId === e.kanrenshaSeijidantaiAddressId)[0];
...
function recieveInputPortalAddressInterface(addressDto: InputAddressDtoInterface) {
    ...
    const tempEntity: KanrenshaSeijidantaiAddressEntityInterface | undefined
        = listPersonAdsdress.value.filter((e) => storedId.value === e.kanrenshaSeijidantaiAddressId)[0];
...
<template>
    ...
    <div class="one-line-scroll">
        <table>
            ...
            <tbody v-for="entity in listPersonAdsdress" :key="entity.kanrenshaSeijidantaiAddressId">
                <tr>
                    ...
                    <td><button @click="onPortalAddressInput(entity.kanrenshaSeijidantaiAddressId)">編集</button></td>
                </tr>
            </tbody>
        </table>
    </div>
