...
import type { UpdateWkTblMinSeijidantaiResultDtoInterface } from '../../dto/wktbl_min/updateWkTblMinSeijidantaiResultDto';
...
const MESS_PAGE_NAME: string = "関連者政治団体マスタ最小ワークテーブル編集";
...
function onSearchSeijidantai() {
    ...
    getAuthorizedPromiseArea().then(token => {
        const url = urlBack + "/regist-bulk-master-min/search-seijidantai";
        const method = "POST";
...
function onEditData(editId: number) {
    ...
    const tempIndex = seijidantaiResultDto.value.listWktblSeijidantai.findIndex((e) => e.wkTblKanrenshaSeijidantaiAddMinId === editId);
...
function onEditUpdate() {
    ...
    getAuthorizedPromiseArea().then(token => {
        const url = urlBack + "/regist-bulk-master-min/update-seijidantai";
        const method = "POST";
        ...
        fetch(url, { method, headers, body })
            .then(async (response) => {
                const resultDto: UpdateWkTblMinSeijidantaiResultDtoInterface = await response.json();
